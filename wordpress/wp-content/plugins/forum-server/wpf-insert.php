...
if(isset($_POST['add_topic_submit'])){
	...
	elseif($content == ""){
		...
		$sql_thread = "INSERT INTO $vasthtml->t_threads (last_post, subject, parent_id, `date`, status, starter) VALUES(NOW(), '".mysql_real_escape_string(stripslashes($subject))."', ".(int)$forum_id.", NOW(), 'open', ".(int)$user_ID.")";		
		$wpdb->query($sql_thread);
		$id = $wpdb->insert_id; 
		$sql_post = "INSERT INTO $vasthtml->t_posts (text, parent_id, `date`, author_id, subject) VALUES('".mysql_real_escape_string(stripslashes($content))."', ".(int)$id.", NOW(), ".(int)$user_ID.", '".mysql_real_escape_string(stripslashes($subject))."')";
		$wpdb->query($sql_post);
	}
	if(!$error){
...
if(isset($_POST['add_post_submit'])){
	...
	if(!$error){
		...
		$count = $wpdb->get_var("SELECT COUNT(*) FROM $vasthtml->t_posts WHERE parent_id = ".(int)$thread);
...
if(isset($_POST['edit_post_submit'])){
	...
	$sql = "UPDATE $vasthtml->t_posts SET text = '".mysql_real_escape_string(stripslashes($content))."', subject = '".mysql_real_escape_string(stripslashes($subject))."' WHERE id = ".(int)$edit_post_id;
	$wpdb->query($sql);
